...
class NeuralNetCreator:
    ...
    '''
    def train(self,tsc,num_epochs, training_percent):
        training_set, testing_set = tsc.getTrainingTestingIndices(training_percent)
        training_set, validation_set = tsc.getTrainingTestingIndices(training_percent,subset=training_set)
        for epoch in range(num_epochs):
            # In each epoch, we do a full pass over the training data:
            train_err = 0
            train_batches = 0
            start_time = time.time()
            for datasetindex in range(len(tsc.names)):   
                for minibatch in tsc.getMiniBatches(datasetindex, training_set[datasetindex], 500):
                    ...
                    train_err += self.train_fn(minibatch,target)
                    train_batches += 1
            ...
            val_batches = 0
            for datasetindex in range(len(tsc.names)):
                for minibatch in tsc.getMiniBatches(datasetindex, validation_set[datasetindex], 500):
                    target=np.full((len(minibatch)),tsc.target_dict[tsc.names[datasetindex]],dtype=np.int)
                    err, acc = self.val_fn(minibatch, target)
                    val_err += err
                    val_acc += acc
                    val_batches += 1
        ...
        test_batches = 0
        for datasetindex in range(len(tsc.names)):
            for minibatch in tsc.getMiniBatches(datasetindex, testing_set[datasetindex], 500):
                target=np.full((len(minibatch)),tsc.target_dict[tsc.names[datasetindex]],dtype=np.int)
                err,acc = self.val_fn(minibatch,target)
                test_err += err
                test_acc += acc
